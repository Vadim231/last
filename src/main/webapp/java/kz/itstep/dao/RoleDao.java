...
public class RoleDao extends AbstractDao<Role> {
    ...
    private static final String INSERT = "insert into roles (title) values(?)";
    private static final String DELETE_ID = "delete from roles where id=";
    @Override
    ...
    public List<Role> findAll() {
        ...
        try(Statement statement = connection.createStatement(); ResultSet resultSet = statement.executeQuery(SQL_SELECT_ALL)){
            while (resultSet.next()){
                Role role = new Role(resultSet.getString("title"));
    ...
    private Role setRoleParameters(ResultSet resultSet){
        ...
        try {
            role.setId(resultSet.getInt("id"));
            role.setName(resultSet.getString("title"));
        } catch (SQLException e) {
            logger.error(e.getMessage());
        }
